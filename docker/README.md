...
| [`Containerfile.kenya_hr`](Containerfile.kenya_hr) | Stage-2 image that layers this repo's custom apps (`apps/kenya_hr`, `apps/kenya_procurement`, `apps/kenya_fleet`) onto the built image. |
...
> **Note:** `apps.json` is intentionally valid strict JSON. The Kenya HR,
> Kenya Procurement and Kenya Fleet apps are sub-directories of this
> repository, so they cannot be referenced by URL here — they are copied into
> the image by `Containerfile.kenya_hr`.