...
The Kenya HR app ships with roles but lets the administrator control exactly
what each role can do. This page documents the recommended matrix and how the
custom doctypes enforce access.
...
| `Employee` (standard) | All staff | Self-service: apply for leave, view own record |

## Where permissions live

1. **In code** — the `permissions` blocks inside the doctype JSON, e.g. for
   `Staff Promotion` and `Kenya HR Settings`. These are applied by `bench migrate`.
...
3. **In the UI** — HRMS entities (`Employee`, `Leave Application`) use the
